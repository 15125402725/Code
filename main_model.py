...
def plot_selected_feature_descriptions(
    ...
    # Continuous variables: standardize over the full cohort for descriptive display only.
    scaled_cont_df = None
    if continuous_features:
        ...
        scaled_cont_df.to_csv(
            os.path.join(
                full_output_dir,
                "selected_continuous_features_standardized_full_cohort.csv",
            ),
            index=False,
        )
    ...
    df[selected_features + [target_col]].to_csv(
        os.path.join(
            full_output_dir,
            "selected_features_raw_full_cohort.csv",
        ),
        index=False,
    )
...
def main():
    ...
    for idx, (name, pipeline) in enumerate(fitted_models.items()):
        ...
        gmean = np.sqrt(rec * specificity) if np.isfinite(specificity) else np.nan
    ...
    roc_legend.get_frame().set_linewidth(0.8)
    plt.tight_layout()
    plt.savefig(
        os.path.join(roc_curves_dir, "all_models_roc.pdf"),
        format="pdf",
        dpi=600,
        bbox_inches="tight",
    )
    ...
    print("\n" + "=" * 70)
    print("Starting descriptive analysis of selected features...")
    ...
    # ----- Figure 1: prediction-set membership matrix for first 50 test samples ----
    # This replaces the previous point-based individual_intervals figure.
    # Interpretation:
    #   - Blue cell: the class is included in the conformal prediction set.
    #   - Light-gray cell: the class is not included.
    #   - Black star: the observed true class for that sample.
    # Therefore, singleton, multiple-label, empty, and coverage-error cases can
    # be distinguished directly from the same matrix.
    ...
    membership_cmap = ListedColormap(["#BFD8EB","#F5C9B6"])
    ax1.imshow(
        membership,
        aspect="auto",
        interpolation="none",
    ...
    # Overlay true-class markers. A star on a gray cell immediately identifies
    # a sample whose true class is not covered by the prediction set.
    for sample_idx in range(n_show):
    ...
    # Thin white cell boundaries improve readability without visual clutter.
    ax1.set_xticks(np.arange(-0.5, n_show, 1), minor=True)
    ax1.set_yticks(np.arange(-0.5, len(class_list), 1), minor=True)
    ax1.grid(which="minor", color="white", linestyle="-", linewidth=0.65)
    ax1.tick_params(which="minor", bottom=False, left=False)

    # Show every fifth sample index to avoid label crowding.
    major_x = np.arange(0, n_show, 5)
    ax1.set_xticks(major_x)
    ax1.set_xticklabels((major_x + 1).astype(int))
    ...
    legend_elements = [
        Patch(facecolor="#F5C9B6", edgecolor="none", label="Included in prediction set"),
        Patch(facecolor="#BFD8EB", edgecolor="#b0b0b0", label="Not included"),
        Line2D(
            [0],
            [0],
            marker="*",
    ...
    matrix_legend.get_frame().set_linewidth(0.8)

    plt.tight_layout()
    plt.savefig(
        os.path.join(BASE_OUTPUT, "prediction_set_matrix.pdf"),
        format="pdf",
        dpi=600,
        bbox_inches="tight",
    )
