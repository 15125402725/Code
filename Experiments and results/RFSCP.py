...
class AdvancedConformalPredictor:
    ...
    def predict_with_confidence(self, model, X_cal, y_cal, X_test):
        ...
        self.q_hat = np.sort(cal_scores)[max(0, k - 1)]
    ...
    def plot_error_distribution(self, model, X_cal, y_cal):
        ...
        plt.axvline(self.q_hat, color='r', linestyle='--', linewidth=2,
                    label=f'Critical value (α={self.alpha:.2f})')
        plt.title("Nonconformity Scores Distribution", fontsize=14)
        plt.xlabel("1 - P(y_true)", fontsize=12)
        ...
        plt.axhline(1 - self.alpha, color='green', linestyle=':', linewidth=2, label='1-α level')
        plt.axvline(self.q_hat, color='red', linestyle='--', linewidth=2)
        plt.title("Quantile Position", fontsize=14)
...
class ResultVisualizer:
    ...
    def plot_metrics(self, metrics_df, filename="cv_metrics.png"):
        ...
        sns.boxplot(data=metrics_df, orient='h', palette='Blues')
        plt.title('Cross-Validation Metrics Distribution', fontsize=14)
        plt.tight_layout()
    ...
    def plot_roc_pr(self, y_true, y_proba, filename="roc_pr.png"):
        ...
        plt.title('ROC & Precision-Recall Curves (RF_SCP)', fontsize=14)
    ...
    def plot_coverage_vs_set_size(self, confidence_levels, coverage_rates, avg_set_sizes,
        ...
        color = 'tab:blue'
        ax1.set_xlabel('Confidence Level (1-α)', fontsize=12)
        ax1.set_ylabel('Coverage Rate', color=color, fontsize=12)
        ax1.plot(confidence_levels, coverage_rates, '-o', color=color,
                 linewidth=2, markersize=8, label='Coverage Rate')
        ...
        ax2 = ax1.twinx()
        color = 'tab:red'
        ax2.set_ylabel('Average Set Size', color=color, fontsize=12)
        ax2.plot(confidence_levels, avg_set_sizes, '--s', color=color,
                 linewidth=2, markersize=8, label='Average Set Size')
        ax2.tick_params(axis='y', labelcolor=color)

        plt.title('Coverage Rate vs. Prediction Set Size (Random Forest)', fontsize=14)
...
if __name__ == "__main__":
    ...
    try:
        X_train, y_train, X_cal, y_cal, X_test, y_test = preparer.load_and_split(
            "PRAD_LUAD_filtered_gene_data_SIS.csv"
        )
